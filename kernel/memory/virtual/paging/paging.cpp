...
static uint16_t get_PML4_Index(const uint64_t address) {
    return (address >> (PAGE_SHIFT + 9 * 3)) & TABLE_IDX_MASK;
}

static uint16_t get_PDPT_Index(const uint64_t address) {
    return (address >> (PAGE_SHIFT + 9 * 2)) & TABLE_IDX_MASK;
}

static uint16_t get_PD_Index(const uint64_t address) {
    return (address >> (PAGE_SHIFT + 9)) & TABLE_IDX_MASK;
}

static uint16_t get_PT_Index(const uint64_t address) {
    return (address >> PAGE_SHIFT) & TABLE_IDX_MASK;
}

static uint64_t get_start_of_tree() {
    uint64_t cr3;
    __asm__ volatile("mov %%cr3, %0" : "=r"(cr3));
    return cr3;
}

static void drop_page_from_TLC(const uint64_t pageVirtualAddr) {
    __asm__ volatile("invlpg (%0)" : : "r"(pageVirtualAddr) : "memory");
}

static void handle_read_write_flag(const PagingFlags flags, PageTableEntry* pt3_PTI) {
    if (flags & PagingFlags::PAGE_FLAG_WRITE) {
        pt3_PTI->ReadWrite = 1;
        return;
    }
...
static void handle_cache_disable_flag(const PagingFlags flags, PageTableEntry* pt3_PTI) {
    if (flags & PagingFlags::PAGE_FLAG_CACHE_DISABLE) {
...
static uint64_t get_or_create_next_table(PageTable* prevTableAddr, const uint16_t Idx) {
