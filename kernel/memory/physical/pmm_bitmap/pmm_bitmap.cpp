...
void get_free_location_for_bitmap() {
    pmm_bitmap.count = highestAddress / PAGE_SIZE;
    uint64_t reqSize = divide_round_up(pmm_bitmap.count, 8);
    ...
    for (uint64_t i = 0; i < memoryRegionCount; i++) {
        ...
        if (entry->type == LIMINE_MEMMAP_USABLE) {
            if (entry->length >= reqSize) {
                ...
                pmm_bitmap.bitmap = (unsigned char*) entry->base + hhdm_offset;
                break;
            }
        }
    }

    memory_fill(pmm_bitmap.bitmap, 0xFF, reqSize); // mark everything as filled
    init_bitmap_data();
